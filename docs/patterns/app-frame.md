...
| `topbar-right`     | Actions on the thing the page is showing, the notification bell, the account menu when there is no rail | Navigation, a status the page could show inline            | The right side is "what can I do here". Navigation there duplicates the rail. For an action on one section rather than the whole page, see [Where the primary action goes](/patterns/team-management#where-the-primary-action-goes) |
