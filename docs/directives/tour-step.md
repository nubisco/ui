...
`v-nb-tour-step` tags an element as a [walkthrough](/components/walkthrough) target. All it does is stamp a `data-nb-tour-step` attribute, which the tour resolves by id.
