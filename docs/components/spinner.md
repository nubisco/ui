...
| Situation                                                              | Use                                                      |
| ---------------------------------------------------------------------- | -------------------------------------------------------- |
| You know the total (files, bytes, steps)                               | [`NbProgressBar`](/components/progress-bar) with `value` |
| You know nothing but the operation blocks a region or the page         | `NbSpinner`                                              |
| A named action is running and its result must be confirmed             | [`NbInlineLoading`](/components/inline-loading)          |
| A page or list is filling in for the first time and the shape is known | [`NbSkeleton`](/components/skeleton)                     |
...
[`NbSkeleton`](/components/skeleton) shaped like what is coming. Do not
...
[`NbInlineLoading`](/components/inline-loading), because the load is only
...
running. [`NbButton`](/components/button/button) has a `loading` prop that swaps its
...
That is the same arbitration [`NbModal`](/components/modal) already uses to
...
and [`NbToaster`](/components/toaster)'s live regions and stack are
...
1. Learn a number and switch to [`NbProgressBar`](/components/progress-bar).
...
- **Don't** put a spinner where the content shape is already known. A list that
  ...
  [`NbSkeleton`](/components/skeleton) rows.
- **Don't** confirm success with a spinner disappearing. Silence is not
  confirmation. Use [`NbInlineLoading`](/components/inline-loading).
...
- **The live region is mounted empty and filled on the next task.** A region
  ...
  and the reason [`NbInlineLoading`](/components/inline-loading) keeps its
...
- `decorative` removes the role, the live region and the label, and marks the
  ...
  announces the state, which is what [`NbInlineLoading`](/components/inline-loading)
