...
`NbAccordion` is a stack of headings that reveal their content when clicked. It is the plain disclosure for page body: [`NbShellPanel`](/components/shell-panel) looks similar but is an inspector section, carrying a size control and expecting a panel rail around it.
...
- **Don't** use it for nested hierarchy. Reach for [`NbTree`](/components/tree) instead.
