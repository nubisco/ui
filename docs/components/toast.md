...
**You almost certainly want [`NbToaster`](/components/toaster) instead.** The host owns everything a notification needs beyond the look of one message: the queue, the shared clock, the visible cap, pause on hover and focus, announcement, dismissal and where focus goes afterwards. Mount `<NbToaster />` once, call [`useToast()`](/composables/use-toast) from anywhere, and you never render `NbToast` yourself.
...
None of that was careless. It is what a container costs to get right, which is why it is library code now. The build-your-own recipe has been removed rather than corrected. If you have a local `Toaster.vue`, the [migration steps](/components/toaster#replacing-a-hand-built-host) are four short ones.
:::
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| A validation error                                               | [`NbMessage`](/components/message) on the field                                       | The fix happens at the field. A toast makes the user remember which one                               |
| A failure that belongs to the page or the form                   | [`NbBanner`](/components/banner) `variant="inline"`, in place                         | The user has to see the failure and the thing that failed together, for as long as it takes to fix it |
| A condition that stays true while the page is open               | [`NbBanner`](/components/banner) `variant="callout"`                                  | A standing fact that expires has been told to the user exactly once, at random                        |
| A question that must be answered before anything continues       | [`useConfirm()`](/composables/use-confirm)                                            | A toast cannot block, and it cannot be answered. A toast that "confirms" something confirms nothing   |
| News that outlives the moment and has to be findable later       | [`NbNotificationCenter`](/components/notification-center)                             | Three applications hand-built a bell (312, 343 and 308 lines) because this distinction had no home    |
| Work that is still in progress on this screen                    | [`NbInlineLoading`](/components/inline-loading) or [`NbSpinner`](/components/spinner) | Progress belongs next to the thing making progress ([Status indicators](/patterns/status-indicators)) |
...
If yes, it is not a toast at all. "Your trial ends in three days", "this environment is not production", "two members are still awaiting approval" are all true for as long as the page is, so they belong in [`NbBanner`](/components/banner) `variant="callout"`, which cannot be dismissed because dismissing it would not make it untrue. If the warning is about the form or the page in front of the user, it is `variant="inline"` on that page.
...
Never let a toast be the only route to an action. A toast with an `Undo` is a shortcut offered to someone who is already looking at it; the same undo must exist somewhere with a lifetime, or the action needed a [confirmation](/composables/use-confirm) rather than a toast.
:::
...
- [`NbToaster`](/components/toaster) is the host, and the page to read first.
- [`useToast()`](/composables/use-toast) is the API every call site uses.
- [`NbBanner`](/components/banner) for a message that belongs to the page rather than to a moment, and [`NbMessage`](/components/message) for one about a single field.
- [`NbNotificationCenter`](/components/notification-center) for news that has to be findable later.
