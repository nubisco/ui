...
- **The shape is unknown.** A skeleton is a promise about the layout. If the
  ...
  [`NbSpinner`](/components/spinner) and do not promise.
...
- **A load with a measurable total.** That is
  [`NbProgressBar`](/components/progress-bar), which can say how far along it
...
- **A refresh of content already on screen.** Replacing rendered rows with grey
  ...
  [`NbSpinner`](/components/spinner) with `overlay="container"` over them.
...
[`NbEmptyState`](/components/empty-state) when the response is legitimately
empty, and for a [message or banner](/components/banner) when the request
...
[`useInlineLoading`](/components/inline-loading), this is already done for
...
- **The region is mounted empty and the label is written on the next task.** A
  ...
  watched. [`NbSpinner`](/components/spinner) does the same thing for the
  same reason.
...
- **The message is fired on any unmount, including a failed one.** The component
  ...
  [`useInlineLoading`](/components/inline-loading), which does it for you.
