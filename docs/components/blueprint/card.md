...
`NbBlueprintCard` is a node card designed to live inside an [`NbBlueprint`](/components/blueprint/overview) canvas. It renders a card with an identity rail down its left edge, typed input/output ports, a category label, an optional enable toggle, and an optional remove button. It is presentational: the parent owns the card's position and the connections between cards.
...
For a complete example wiring cards and ports into an `NbBlueprint` canvas, see the [Blueprint](/components/blueprint/overview) docs.
