...
`NbBlueprintBackground` is an optional, drop-in backdrop for an [`NbBlueprint`](/components/blueprint/overview) canvas: a `grid`, `dots`, or `lines` pattern with full control over colour, spacing, and thickness. It reads the camera from [`useBlueprint()`](/components/blueprint/overview#the-useblueprint-composable) and pans/zooms with the scene, so it needs no wiring from the host.
