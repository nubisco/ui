...
`NbBlueprintMinimap` is an optional overview for an [`NbBlueprint`](/components/blueprint/overview) canvas: it draws each card as a small rectangle plus a rectangle for the current viewport, and lets the user click or drag to recenter. It reads the camera through [`useBlueprint()`](/components/blueprint/overview#the-useblueprint-composable) and takes the same `cards` array you pass to the blueprint (windowed API).
