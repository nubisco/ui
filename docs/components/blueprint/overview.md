...
For full control — per-instance colours, gap, thickness, a custom secondary grid, or an entirely custom backdrop — drop [`NbBlueprintBackground`](/components/blueprint/background) (or any element) into the `#background` slot instead. See [Background](/components/blueprint/background).
...
| `background` | (none)                              | Backdrop layer, drawn behind the scene. Place [`NbBlueprintBackground`](/components/blueprint/background) or any element here; when set it replaces the built-in `background` prop. See [Background](/components/blueprint/background). |
...
Most of this is already wrapped by [`NbBlueprintControls`](/components/blueprint/controls) and [`NbBlueprintMinimap`](/components/blueprint/minimap); reach for `useBlueprint()` directly when building custom chrome.
...
| `zoomIn`, `zoomOut`                                                                                                                          | `() => void`                                     | Step zoom, anchored at the viewport center.                  |
| `viewportSize`                                                                                                                               | `Ref<{ w: number; h: number }>`                  | Live container size in screen px (used by the minimap).      |
