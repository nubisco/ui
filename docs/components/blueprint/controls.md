...
`NbBlueprintControls` is an optional, NubiscoUI-styled toolbar for an [`NbBlueprint`](/components/blueprint/overview) canvas: zoom, fit, center, auto-layout, and a contextual alignment cluster. It is wired entirely to the blueprint through [`useBlueprint()`](/components/blueprint/overview#the-useblueprint-composable), so it needs no event plumbing from the host.
...
For full custom controls, build your own with [`useBlueprint()`](/components/blueprint/overview#the-useblueprint-composable).
