...
The dark theme resolves the same roles to lighter tints, see [Chart Color](/components/charts/color). These are chart roles rather than the `--nb-c-status-*` inks because a Gantt bar is a large fill on a busy grid; pass `statusColors` to use the status hues instead.
