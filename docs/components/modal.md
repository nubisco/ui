...
It is one of exactly two dialogs the library ships. The other is [`NbConfirm`](/components/confirm), and it is the only shape a destructive confirmation may take. If you are about to build a confirmation out of `NbModal`, stop and read [When not to use](#when-not-to-use).
...
The correction is not a corrected footer. It is [`useConfirm()`](/composables/use-confirm). What is left here is the footer contract, which that example also got wrong, and which every dialog on this page needs.
:::
...
| ------------------------------------------------------------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| A destructive confirmation                                    | [`useConfirm()`](/composables/use-confirm)                                     | Every hand-built one in the fleet lost the same five things. See below                                                      |
...
| The result of something that just finished                    | [`useToast()`](/composables/use-toast)                                         | A result is news, not a question. Nobody has to answer it, so nobody should have to dismiss it                              |
| A condition that stays true while the page is open            | [`NbBanner`](/components/banner) `variant="callout"`                           | A standing fact that interrupts once and vanishes has been told to the user exactly once, at random                         |
| A failure that belongs to the page or the form                | [`NbBanner`](/components/banner) `variant="inline"`, in place                  | The user has to see the failure and the thing that failed at the same time                                                  |
| Something wrong with one field                                | [`NbMessage`](/components/message)                                             | The field is where the fix happens                                                                                          |
| Properties of a selected object, edited repeatedly            | [`NbShellPanel`](/components/shell-panel) ([Inspectors](/patterns/inspectors)) | Selecting the next object should not mean reopening a dialog. The object and its editor stay on screen together             |
| A whole task with its own sub-steps                           | A route, or [`NbStepper`](/components/stepper) inside one                      | If it deserves a back button, it deserves a URL                                                                             |
| Marketing, onboarding, a tour, a "what's new"                 | Nothing, or [`NbWalkthrough`](/components/walkthrough)                         | A dialog nobody opened is an interruption, not a conversation. Dialogs are user-initiated                                   |
...
| ----------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------- |
| **Confirmation**  | An irreversible or expensive act, answered yes or no                    | <kbd>Esc</kbd>, Cancel, scrim, X. All four mean no. None mean yes | Cancel, then a `variant="danger"` commit  | [`useConfirm()`](/composables/use-confirm), never `NbModal`    |
...
**A dialog with nothing to decide is not a dialog.** If the user's only possible answer is "fine", the message is news: [`useToast()`](/composables/use-toast) for a moment, [`NbBanner`](/components/banner) for a condition. An OK-only dialog is an interruption that charges a click for nothing.
...
- **A timer or a route entering.** A dialog nobody opened is an interruption. If the page has something to say on arrival, it is an [`NbBanner`](/components/banner) `variant="callout"`, which stays true for as long as the page does.
...
**A body that is still fetching opens anyway.** Delaying the open so the content can arrive gives the user a button that appears not to have worked, which is how a fleet application ended up with a double-click on every row action. Open immediately and put [`NbSkeleton`](/components/skeleton) where the content will be, at the shape the content will have, so nothing jumps when it lands:
...
If the fetch comes back with nothing, say so with [`NbEmptyState`](/components/empty-state) inside the body. A blank dialog and a loading dialog must not look the same: one audited application renders a single `.empty-text` for both, so a failed fetch reads as "no contacts yet".
...
- [`NbConfirm`](/components/confirm) and [`useConfirm()`](/composables/use-confirm) for anything irreversible.
- [`useToast()`](/composables/use-toast) for the result, and for offering undo instead of asking.
- [`NbShellPanel`](/components/shell-panel) and [Inspectors](/patterns/inspectors) for editing a selection in place.
- [Forms](/patterns/forms) for what a dialog may and may not contain.
- [`NbSkeleton`](/components/skeleton) and [`NbEmptyState`](/components/empty-state) for a body that is still loading or came back empty.
...
The header X carries a hardcoded `aria-label="Close"`. Unlike [`NbToast`](/components/toast), which exposes `closeLabel`, there is nothing to translate it with. Render your own control in `#header` if that matters to you.
:::
