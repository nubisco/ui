...
`flush` drops the radius and the side borders so the banner meets both edges of the region holding it. This is what `NbShell`'s [`#notification` slot](/components/shell) expects.
