...
`NbPagination` is the footer companion for [`NbDataTable`](/components/data-table), usable standalone. It is fully controlled: it emits `update:page` / `update:pageSize` and renders whatever `page`, `pageSize` and `total` you pass back, so the same wiring drives client-side slicing or server-side paging.
