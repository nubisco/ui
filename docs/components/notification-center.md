...
| ----------- | -------------------- | ------------------------------------------------------------------------------- |
| **Loading** | `loading`            | Three [`NbSkeleton`](/components/skeleton) rows, `aria-busy`, one announcement. |
...
- <kbd>Escape</kbd> arrived while focus was somewhere else on the page. The panel is not modal, so the page underneath is live and that keystroke is usually not about the bell at all: the realistic sender is a nested overlay opened from a row action or from `#footer` (an [`NbModal`](/components/modal), a teleported [`NbSelect`](/components/select) listbox) dismissing itself in the same tick. The panel goes; the caret stays. The exception is the trigger already holding focus, where "leave it alone" and "put it back" are the same thing.
...
When an event genuinely needs to interrupt, raise an [`NbToast`](/components/toast) for it and let it also land in the centre, so the toast is the interruption and the panel is the record. Reserve `role="alert"` for what would be worth a person tapping you on the shoulder.
...
1. **Does anything arrive while the user is not looking?** If every event in your product is the direct result of a button the user just pressed, an [`NbToast`](/components/toast) or an [`NbInlineLoading`](/components/inline-loading) beside that button says it better, and a bell that only ever mirrors your own clicks trains people to stop opening it.
...
- **Anything blocking.** If the user cannot proceed until they respond, that is an [`NbConfirm`](/components/confirm) or an [`NbModal`](/components/modal), in front of them, now. A bell badge is not consent.
- **Anything catastrophic and current.** A production incident, an expired card, a locked account: those are an [`NbBanner`](/components/banner) on the page they affect. A person who has not opened the panel has not been told.
...
- The list carries an explicit `role="list"` and each row an explicit `role="listitem"`. Both are needed: WebKit strips list semantics from a list styled with `list-style: none`, which every list in this library is, and without them VoiceOver never says "list, 12 items". [`NbReorderList`](/components/reorder-list) carries the same explicit `role="list"`, though not the item role, so this is the library's convention taken one step further rather than a local invention.
