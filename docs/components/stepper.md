...
| [`NbWalkthrough`](/components/walkthrough)  | The user is being shown around UI that already exists: a coach-mark tour of the app.                |
| [`NbProgressBar`](/components/progress-bar) | A machine is doing the work and the user is waiting: an upload, an import, a build.                 |
| [`NbTabs`](/components/tabs)                | The sections are peers in any order, and only one is on screen at a time.                           |
...
Under about 672px (the `md` breakpoint in [the grid](/components/grid)),
...
- **Waiting on the step contents.** Render the stepper immediately, and put
  [`NbSkeleton`](/components/skeleton) or
  [`NbInlineLoading`](/components/inline-loading) inside the step panel. The
