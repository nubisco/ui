...
[`NbToast`](/components/toast) renders a single message and stays exactly what it was. `NbToaster` decides when it appears and when it goes.
...
| [`NbBanner`](/components/banner)                       | Something is true about this page or task and stays on screen.           |
| [`NbMessage`](/components/message)                     | One field has something to say about its own value.                      |
| [`NbConfirm`](/components/confirm)                     | Nothing else may proceed until the user answers.                         |
| [Notification centre](/components/notification-center) | The news outlives the moment and has to be findable later.               |
...
Keep the message to one or two lines. If it needs more, it is not a toast: put it on the page, or open a [confirm](/components/confirm).
...
The toast itself is tokenised in [`NbToast`](/components/toast#tokens). A test reads the style block of both files and fails on any hex value, any `rgba()` and any `var(--nb-*, fallback)`.
...
- Do not put a toast in place of a form error. A field's error belongs to the field ([`NbMessage`](/components/message)).
