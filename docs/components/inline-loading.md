...
[`NbSpinner`](/components/spinner) or
[`NbSkeleton`](/components/skeleton) that is unmounted by the same state
...
- **Don't** hide a retry in the label string. If the failure needs an action,
  ...
  [`NbButton`](/components/button/button) next to the component.
...
[banner](/components/banner), not a status line.
...
- The operation has a measurable total: [`NbProgressBar`](/components/progress-bar).
- The whole region is unusable while it runs: [`NbSpinner`](/components/spinner)
  with `overlay="container"`.
- The page is filling in for the first time: [`NbSkeleton`](/components/skeleton).
- The outcome must survive navigation, or the user may have looked away:
  a [toast](/components/toaster). Inline confirmation is for results the user
  is looking at.
...
- Reduced motion is inherited from [`NbSpinner`](/components/spinner): under
...
[`NbSkeleton`](/components/skeleton) under "when the fetch fails" for the
